...
for car, value in car_data.items():
    year, engine_size, price = value[1], value[2], value[4]
    if year >= search_criteria[0] and engine_size >= search_criteria[1] and price <= search_criteria[2]:
        matching_cars.append((car, value))

sorted_matching_cars = sorted(matching_cars, key=lambda x: x[1][4])
for car in sorted_matching_cars[:5]:
    print(car)